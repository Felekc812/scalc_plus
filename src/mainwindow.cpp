...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->Button_dot, SIGNAL(clicked()), this, SLOT(numbers()));

  connect(ui->Button_x, SIGNAL(clicked()), this, SLOT(butten_x()));

  connect(ui->Button_skob_don, SIGNAL(clicked()), this, SLOT(numbers()));
  connect(ui->Button_skob_up, SIGNAL(clicked()), this, SLOT(numbers()));
...
void MainWindow::on_Button_funk_clicked() {
  ...
  if (ui->checkBox_3->isChecked()) {
    ...
    int j = prov_form(input.data());
    do {
      input.replace(j - 1, 1, psz_x);
      j = prov_form(input.data());
    } while (j != 0);
  }
  ...
  if (l != 0) {
    ...
    for (X = X_0; X < X_1; X += h) {
      ...
      double reza = 0;
      do {
        input2.replace(l - 1, 1, psz);
        l = prov_form(input2.data());
      } while (l != 0);
      printf("%s \n", input2.data());
      z = calc(input2.data(), &reza);
      printf("X %f  Y %f \n", X, reza);
      x.push_back(X);
      y.push_back(reza);
    }
